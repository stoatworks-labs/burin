...
`tools/make_example_svg.py`; both frames rendered by `rztest`, the offline
harness.</sub>
...
log and in `rztest --doc`, because "the plugin does nothing" and "your file is
...
**Rasterizer** is a source: the drawing on its own background, on its own layer.

**Rasterizer Over** is an effect: the same drawing over the incoming clip, with a
...
Then in Resolume: **Sources → Generators → Rasterizer**, or drop **Rasterizer
...
`build/Rasterizer.ofx.bundle` into `/Library/OFX/Plugins`.
