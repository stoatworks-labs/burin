...
# kVertexShader is a plain constant. The fragment shader is accumulated in
# FragmentShader( bool isEffect ): a base, two chunks only the effect build
# gets, and a tail. Both builds ship, so both are compiled here -- and the
# effect-only chunks are exactly where a shader breaks in one build alone.
chunks = []
for m in re.finditer( r'source\s*\+?=\s*R"\((.*?)\)"', text, re.S ):
	guarded = re.search( r'if\s*\(\s*isEffect\s*\)$', text[ : m.start() ].rstrip() )
	chunks.append( ( guarded is not None, m.group( 1 ) ) )
...
emit( "kVertexShader",
      re.search( r'kVertexShader\s*=\s*R"\((.*?)\)"', text, re.S ).group( 1 ) )
for label, isEffect in ( ( "source", False ), ( "effect", True ) ):
	emit( "FragmentShader_" + label,
	      "".join( g for only, g in chunks if isEffect or not only ) )
SHADERS_PY
